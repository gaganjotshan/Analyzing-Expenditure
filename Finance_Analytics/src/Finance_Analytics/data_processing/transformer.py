...
def structure_dataframe(df, states_row_index, states_col_index):
...
def transform_dataframe(df_structured):
...
def process_files(raw_directory_path):
    ...
    for filename in os.listdir(raw_directory_path):
        if filename.endswith(".csv"):
            ...
            try:
                df_structured = structure_dataframe(df, states_row_index, states_col_index)
                melted_df = transform_dataframe(df_structured)

                # Save each transformed DataFrame separately
                output_file_path = os.path.join(transformed_directory_path, f"{category}_transformed.csv")
